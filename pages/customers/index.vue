                                            ...
                                            <table class="min-w-max w-full table-auto">
                                                ...
                                                <tbody class="bg-white divide-y ">
                                                    ...
                                                    <tr v-for="(customers, index) in customers" :key="index"
                                                        ...
                                                        <td class="py-3 px-8 text-left"> {{ customers.name }}</td>
                                                        <td class="py-3 px-8 text-left"> {{ customers.phone }}</td>
                                                        <td class="py-3 px-8 text-left"> {{ customers.email }}</td>
                                                        <td class="py-3 px-18 text-left">
                                                            <div class="flex item-center ">
                                                                <Nuxt-link :to="`/customers/${customers._id}`"
                                                                ...
                                                                <div @click="deleteProduct(customers._id, index)"
                                                                    class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                                        <path stroke-linecap="round"
...
export default {
    ...
    methods: {
        async getcustomers() {
            await this.$axios.$get("/customers").then(success => {
        ...
        async deleteProduct(id, index) {
            ...
            if (confirmation == true) {
                await this.$axios.$delete(`/customers/${id}`).then(success => {
